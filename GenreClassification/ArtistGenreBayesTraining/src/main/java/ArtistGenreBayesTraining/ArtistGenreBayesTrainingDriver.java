...
public class ArtistGenreBayesTrainingDriver {
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "Artist Genre Bayes Training");

        job.addCacheFile(new Path(genresPath).toUri());

        job.setJarByClass(ArtistGenreBayesTrainingDriver.class);
        job.setMapperClass(ArtistGenreBayesTrainingMapper.class);
        job.setReducerClass(ArtistGenreBayesTrainingReducer.class);
        ...
        FileOutputFormat.setOutputPath(job, new Path(outputPath + "/tmp_Artist_Genre_Bayes_Training"));

        job.waitForCompletion(true);

        copyFile(outputPath + "/tmp_Artist_Genre_Bayes_Training/part-r-00000", outputPath + "/artist_genre_bayes_frequency.txt");
        deletePath(outputPath + "/tmp_Artist_Genre_Bayes_Training", true);
    }
