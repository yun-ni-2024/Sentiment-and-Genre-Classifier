...
public class BayesTrainingDriver {
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "Bayes Training");

        job.addCacheFile(new Path(trainPath).toUri());

        job.setJarByClass(BayesTrainingDriver.class);
        job.setMapperClass(BayesTrainingMapper.class);
        job.setReducerClass(BayesTrainingReducer.class);
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(IntWritable.class);
        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(IntWritable.class);
        job.setOutputFormatClass(CommaSeparatedOutputFormat.class);
        TextInputFormat.addInputPath(job, new Path(lyricsPath));
        FileOutputFormat.setOutputPath(job, new Path(outputPath + "/tmp_Bayes_Training"));

        job.waitForCompletion(true);

        copyFile(outputPath + "/tmp_Bayes_Training/part-r-00000", outputPath + "/bayes_frequency.txt");
        deletePath(outputPath + "/tmp_Bayes_Training", true);
    }
