...
public class FilterDriver {
    public static void main(String[] args) throws Exception {
        ...
        Job job4 = Job.getInstance(conf, "Filter Songs");
        job4.addCacheFile(new Path(inputPath + "/lyrics.txt").toUri());
        job4.addCacheFile(new Path(inputPath + "/genres.txt").toUri());
        job4.addCacheFile(new Path(inputPath + "/users.txt").toUri());
        job4.setJarByClass(FilterDriver.class);
        job4.setMapperClass(FilterSongsMapper.class);
        job4.setReducerClass(FilterSongsReducer.class);
        job4.setMapOutputKeyClass(Text.class);
        job4.setMapOutputValueClass(Text.class);
        job4.setOutputKeyClass(NullWritable.class);
        job4.setOutputValueClass(Text.class);
        TextInputFormat.addInputPath(job4, new Path(inputPath + "/songs.txt"));
        FileOutputFormat.setOutputPath(job4, new Path(outputPath + "/tmp_Filter_Songs"));
