...
public class SongGenreBayesTrainingDriver {
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "Song Genre Bayes Training");

        job.addCacheFile(new Path(genresPath).toUri());

        job.setJarByClass(SongGenreBayesTrainingDriver.class);
        job.setMapperClass(SongGenreBayesTrainingMapper.class);
        job.setReducerClass(SongGenreBayesTrainingReducer.class);
        ...
        FileOutputFormat.setOutputPath(job, new Path(outputPath + "/tmp_Song_Genre_Bayes_Training"));

        job.waitForCompletion(true);

        copyFile(outputPath + "/tmp_Song_Genre_Bayes_Training/part-r-00000", outputPath + "/song_genre_bayes_frequency.txt");
        deletePath(outputPath + "/tmp_Song_Genre_Bayes_Training", true);
    }
